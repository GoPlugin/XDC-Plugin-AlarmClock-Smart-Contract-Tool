...
/**
 * 
 * Import Plugin client from smartcontractkit library. 
 * 
 * PluginClient provides helper utilities to -
...
/**
 *
 * Contract to implement Time Locked Contracts using Plugin Oracle.
 * Appropriate Job should be created in Chinlink using the Alarm Clock task. Refer https://docs.chain.link/docs/plugin-alarm-clock/ for more information on Plugin Alarm Clock.
 ...
 * 3. Once the specified duration is elapsed, Chainkk's Alarm Clock would be triggered 
 * 4. Plugin node will create a transaction on XDC which inturn calls the appropriate callback function in the contract
 * 5. Once the callback function is invoked, locked up XDC will be released to all receipients mentioned in the contract 
 * 
 **/
contract TimeLockedContract is PluginClient {
    ...
    uint256 private _linkFee;
      ...
      * -: jobId - Job Id retrieved from Plugin node. Job should includes Alarm Clock in its pipeline
    ...
    constructor(address sender, address[] receivers, uint256[] funds, address oracleContractAddr, bytes32 jobId, address link, uint256 totalXDC, uint256 linkFee, uint256 durationInSeconds) public {
        ...
        /**
         * Set the LINK Token contract address in PluginClient
         **/
        setPluginToken(link);
    ...
    * This option will be available only when the LINK token is not transferred and Plugin job is not initiated
    *
    * modifier - 
    * 
    ...
    **/
    ...
    function onTokenTransfer(address sender, uint value, bytes data) public {
     ...
     /**
     * 
     * Create a Plugin request to start the alarm job and after
    ...
    function initiateAlarm() public returns (bytes32 requestId) 
    {
         ...
         **/
        Plugin.Request memory request = buildPluginRequest(_jobId, address(this), this.releaseFunds.selector);
        ...
        /**
         * "until" parameter will be read by alarm/sleep task in the Plugin job 
         **/
        request.addUint("until", _unlockDate);
        
        /**
         * Send the request to Plugin node. Appropriate LINK token will be charged to execute this request  
         **/
        return sendPluginRequestTo(_oracleContractAddr, request, _linkFee);
    }
     ...
     * 
     */ 
    function releaseFunds(bytes32 _requestId) onlyOracle public recordPluginFulfillment(_requestId)
    {
        /**
         ...
         * onlyOracle modifier - 
         * 
         * This check would not be required if we receive a callback from Plugin node after the specified duration.
