...
/**
 * Retrieves the PLI token balance of the given address
 * 
 * @param {*} vm - Component with web3 instance
 * @param {*} addr - Address to get the PLI token balance 
 * 
 * @returns - PLI token Balance 
 */
async function getLINKBalance(vm, addr) {
  try {
    const linkContract = await new vm.$web3js.eth.Contract(getNetworkConfig(vm).PLI_TOKEN_ABI, getNetworkConfig(vm).PLI_TOKEN_CONTRACT_ADDR);
...
async function transferXDC(vm, config) {
  try {
    ...
    console.log("tlwFactoryContract",tlwFactoryContract);
    let receivers = []
    let funds = []
    ...
    console.log("test log",receivers, funds, getNetworkConfig(vm).ORACLE_CONTRACT_ADDR, vm.$web3js.utils.fromAscii(getNetworkConfig(vm).PLUGIN_JOB_ID), getNetworkConfig(vm).PLI_TOKEN_CONTRACT_ADDR, getNetworkConfig(vm).PLUGIN_FEE+'', config.duration)
    return await tlwFactoryContract.methods.newTimeLockedContract(receivers, funds, getNetworkConfig(vm).ORACLE_CONTRACT_ADDR, vm.$web3js.utils.fromAscii(getNetworkConfig(vm).PLUGIN_JOB_ID), getNetworkConfig(vm).PLI_TOKEN_CONTRACT_ADDR, getNetworkConfig(vm).PLUGIN_FEE+'', config.duration);
  }catch(err) {
    console.log(err)
    vm.rpcInProgress = false
    common.notifyError('Error processing request')
...
async function initiateTimeLockedWallet(vm, config) {
  try {
    ...
    return await tlwContract.methods.initiateAlarm(config.duration, getNetworkConfig(vm).PLUGIN_FEE+'');
  }catch(err) {
    console.log(err)
    vm.rpcInProgress = false
    common.notifyError('Error processing request')
...
/**
 * 
 * Method to transfer PLI token to the contract. PLI token is required for Chainlink oracle node to execute the configured job.
 * Once the link token is transferred,  
 * 
 * @param {*} vm - Component with web3 instance
 * @param {*} config - Timelocked wallet contract address, PLI token value
 * 
 * @returns - Execution status 
 */
async function transferLinkToken(vm, config) {
  try {
    const tlwFactoryContract = await new vm.$web3js.eth.Contract(getNetworkConfig(vm).PLI_TOKEN_ABI, getNetworkConfig(vm).PLI_TOKEN_CONTRACT_ADDR);
