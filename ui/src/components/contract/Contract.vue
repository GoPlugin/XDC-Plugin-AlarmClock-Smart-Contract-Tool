    ...
    <div class="container">
      <div id="create_contract" class="create-contract">
        <ul class="nav">
          ...
          <li>
            <a class="nav-link" href="#step-2"> Initiate Plugin Job </a>
          </li>
          <li>
            ...
            <form id="step_form_1">
              <div class="row">
                ...
                <div class="col-sm-12 col-md-8 text-center">
                  <label class="form-label contract-inp-header">Transfer PLI token to initiate the Plugin job. Your current balance is <b>{{ data.linkBalance }} PLI </b>.</label>
                  <label class="form-label contract-inp-header">By proceeding <b>0.1 PLI</b> token will be transferred to the Time Locked Contract at address <b>{{data.walletContractAddress}}</b>.</label>
                </div>
                <div class=".d-none .d-md-block .d-lg-none col-md-2"></div>
              </div>
            </form>
...
export default {
  ...
  methods: {
     ...
     * 2. In case of a edit contract (applicable only if PLI token is not transferrd while creating the contract initially), existing contract details will be retrieved from the appropriate network chosen in XinPay
     * 
     */
    async loadContract() {
    ...
    async initiateTransaction(step) {
      switch (step) {
        ...
        case 1:
          /**
           * Second step of the wizard where PLI token will be transferred to the contract
           */
    ...
    */
    ...
    /**
     * Transfer PLI token to the contract
     */
    async _transferLinkToken() {
      ...
      const vm = this;
       /**
       * Transfer PLI token and wait for the confirmation i.e wait till the transaction is complete
       */
      linkRpc.send(
        { from: this.data.address },
        function (error, transactionHash) {
          if (error) {
            common.notifyError("Error transferring PLI token to contract");
            vm.rpcInProgress = false;
            return;
          }
          /**
          ...
          web3Util.waitForReceipt(vm, transactionHash, async function (result) {
             ...
             * Notify user with success message and progress the wizard to next step
             */
            common.notifySuccess("PLI token transferred to contract");
            vm.progressWizard();
          });
        }
      );
