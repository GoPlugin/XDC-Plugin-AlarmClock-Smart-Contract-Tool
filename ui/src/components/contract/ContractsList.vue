            ...
            <tr v-for="(contract, index)  in contracts"  :key="contract.walletAddress">
                ...
                <td>
                    <template v-if="!contract.isLinkTransferred && !contract.isWithdrawn">
                        <a :href="'/contract/'+contract.walletAddress" class="btn btn-sm" role="button" title="Transfer Pli Token" style="font-size: 20px;"><i class="bi bi-folder-symlink"></i></a>
...
/**
 * Component to display the list of contracts created by the logged in user. Provides following actions for each contract only if PLI token is not transferred - 
 * 
 * 1. Transfer PLI token 
...
export default {
    ...
    methods: {
         ...
         * 2. For each contract address retrieve the contract details (i.e. receivers and corresponding XDC, total PLI tokens etc)
         */
        async load() {
            this.initDataTable()
        ...
        /**
         * Load the contract details for each contarct (i.e. receivers and corresponding XDC, total PLI tokens etc)
         */
        async getContractDetails() {
            ...
            this.rpcInProgress = false
                        console.log("contract.isLinkTransferred",this.contracts)
         ...
         * This option will be applicable only if PLI token is not transferred to the contract
         */
